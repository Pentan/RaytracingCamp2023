#ifndef LINEARALGEBRA_VECTOR3_INL
#define LINEARALGEBRA_VECTOR3_INL
...
namespace linearalgebra {
    ...
    template<typename FPType>
    inline void Vector3<FPType>::negate(void) {
        x = -x;
        y = -y;
        z = -z;
    }
    ...
    template<typename FPType>
    inline FPType Vector3<FPType>::getMaxComponent(int* outcompid) const {
        if(outcompid) {
            *outcompid = (x > y)? kX : kY;
    ...
    template<typename FPType>
    inline FPType Vector3<FPType>::getMinComponent(int* outcompid) const {
        if(outcompid) {
            *outcompid = (x < y)? kX : kY;
            *outcompid = (v[*outcompid] < z)? *outcompid : kZ;
            return v[*outcompid];
