#ifndef LINEARALGEBRA_MATRIX4_INL
#define LINEARALGEBRA_MATRIX4_INL
...
namespace linearalgebra {
    ...
    inline bool Matrix4<FPType>::invert() {
        ...
        for(i = 0; i < 4; i++) {
            //choose pivot
            int maxrow = i;
            FPType maxabs = std::abs(mrowptr[i]->v[i]);
            for(j = i + 1; j < 4; j++) {
                if(maxabs < std::abs(mrowptr[j]->v[i])) {
                    maxabs = std::abs(mrowptr[j]->v[i]);
                    maxrow = j;
                }
            }
            // swap
    ...
    inline Vector3<FPType> Matrix4<FPType>::mulAndProjectV3(const Matrix4 ml, const Vector3<FPType> vr) {
        Vector4<FPType> v4 = Vector4<FPType>(vr.x, vr.y, vr.z, 1.0);
